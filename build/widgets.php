 ...
 *   underway/widgets/{widget-dir-name}/render
 *   underway/widgets/{widget-dir-name}/widget
 */
function underway_register_widget_modules() {
	$widgets = underway_get_registered_widget_modules();
	if ( empty( $widgets ) ) {
		return;
	}
...
if ( did_action( 'init' ) ) {
	underway_register_widget_modules();
} else {
	add_action( 'init', 'underway_register_widget_modules' );
}
 ...
 *                                          module handles.
 */
function underway_get_registered_widget_modules() {
	static $widgets = null;
		...
		$widgets = array_map(
			static function ( $widget ) {
				$widget['render_module'] = ! empty( $widget['has_render'] )
					? 'underway/widgets/' . $widget['dir_name'] . '/render'
					: null;
				$widget['widget_module'] = ! empty( $widget['has_widget'] )
					? 'underway/widgets/' . $widget['dir_name'] . '/widget'
					: null;
				return $widget;
			},
			$raw
